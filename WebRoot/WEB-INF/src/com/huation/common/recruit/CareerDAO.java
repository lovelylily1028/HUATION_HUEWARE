...
public class CareerDAO extends AbstractDAO {
	...
	public CareerDTO registResult(String applycode) throws DAOException{
		
		String apply_code="";
		String passwd="";
		StringBuffer sb = new StringBuffer();
		QueryStatement sql = new QueryStatement();
		sb.append(" SELECT APPLY_CODE FROM T_CAREER ");
		...
		DataSet ds = null;
		CareerDTO resultDto=new CareerDTO();
	...
	public int[] addHistory(String[] commNos) throws Exception{
		...
		try{
			...
			for(int i=0; commNos != null && i<commNos.length; i++){ 
				...
				log.debug("r_data[5]:"+StringUtil.nvl(r_data[6],""));
				log.debug("r_data[5]:"+StringUtil.nvl(r_data[7],""));
				log.debug("r_data[5]:"+StringUtil.nvl(r_data[8],""));
				*/
				sb.append(" INSERT T_CAREER  (APPLY_CODE,HISTORY_CODE,START_YM,END_YM,WORK_ORG,WORK_DEP,LAST_POSITION,CHARGE_WORK,EMPLOY_TYPE,RETIRE_REASON) 	\n");
				sb.append(" VALUES('"+StringUtil.nvl(r_data[0],"")+"','"+StringUtil.nvl(history_code,"")+"','"+StringUtil.nvl(r_data[1],"")+"','"+StringUtil.nvl(r_data[2],"")+"','"+StringUtil.nvl(r_data[3],"")+"','"+StringUtil.nvl(r_data[4],"")+"','"+StringUtil.nvl(r_data[5],"")+"','"+StringUtil.nvl(r_data[6],"")+"','"+StringUtil.nvl(r_data[7],"")+"','"+StringUtil.nvl(r_data[8],"")+"'); \n");
	...
	public ArrayList historyListView( String apply_code) throws DAOException{
		...
		ArrayList<CareerDTO> arrlist = new ArrayList<CareerDTO>();
		try{			
			sbSql.append(" SELECT APPLY_CODE,HISTORY_CODE,START_YM,END_YM,WORK_ORG,WORK_DEP,LAST_POSITION,CHARGE_WORK,EMPLOY_TYPE,RETIRE_REASON ");
			sbSql.append(" FROM T_CAREER ");
			...
			log.debug("historyListView["+sbSql.toString()+"]");	
			 ...
			 try{
				 ...
				 while(ds.next()){ 
					 ...
					 careerDto.setApply_code(ds.getString("APPLY_CODE"));
					 careerDto.setHistory_code(ds.getString("HISTORY_CODE"));
					 careerDto.setStart_ym(ds.getString("START_YM"));
					 careerDto.setEnd_ym(ds.getString("END_YM"));
					 careerDto.setWork_org(ds.getString("WORK_ORG"));
					 careerDto.setWork_dep(ds.getString("WORK_DEP"));
					 careerDto.setLast_position(ds.getString("LAST_POSITION"));
					 careerDto.setCharge_work(ds.getString("CHARGE_WORK"));
					 careerDto.setEmploy_type(ds.getString("EMPLOY_TYPE"));
					 careerDto.setRetire_reason(ds.getString("RETIRE_REASON"));

					 arrlist.add(careerDto);
				 }
			 }catch(Exception e){
			      e.printStackTrace();
			      log.error(e.getMessage());
