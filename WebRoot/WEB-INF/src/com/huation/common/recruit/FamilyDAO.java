...
public class FamilyDAO extends AbstractDAO {
	...
	public FamilyDTO registResult(String applycode) throws DAOException{
		
		String apply_code="";
		String passwd="";
		StringBuffer sb = new StringBuffer();
		QueryStatement sql = new QueryStatement();
		sb.append(" SELECT APPLY_CODE FROM T_FAMILY_HISTORY ");
		...
		DataSet ds = null;
		FamilyDTO resultDto=new FamilyDTO();
	...
	public int[] addHistory(String[] commNos) throws Exception{
		...
		try{
			...
			for(int i=0; commNos != null && i<commNos.length; i++){ 
				...
				*/
				sb.append(" INSERT T_FAMILY_HISTORY  (APPLY_CODE,HISTORY_CODE,NAME,RELATION,BIRTH_YMD,COHABIT_YN,WORKPLACE) 	\n");
	...
	public ArrayList historyListView( String apply_code) throws DAOException{
		...
		ArrayList<FamilyDTO> arrlist = new ArrayList<FamilyDTO>();
		try{			
			sbSql.append(" SELECT APPLY_CODE,HISTORY_CODE,NAME,RELATION,BIRTH_YMD,COHABIT_YN,WORKPLACE ");
			sbSql.append(" FROM T_FAMILY_HISTORY ");
			...
			log.debug("historyListView["+sbSql.toString()+"]");	
			 ...
			 try{
				 ...
				 while(ds.next()){ 
					 ...
					 familyDto.setApply_code(ds.getString("APPLY_CODE"));
					 familyDto.setHistory_code(ds.getString("HISTORY_CODE"));
					 familyDto.setName(ds.getString("NAME"));
					 familyDto.setRelation(ds.getString("RELATION"));
					 familyDto.setBirth_ymd(ds.getString("BIRTH_YMD"));
					 familyDto.setCohabit_yn(ds.getString("COHABIT_YN"));
					 familyDto.setWorkplace(ds.getString("WORKPLACE"));

					 arrlist.add(familyDto);
				 }
			 }catch(Exception e){
			      e.printStackTrace();
			      log.error(e.getMessage());
	...
	public int deleteHistory(String apply_code) throws DAOException{
		 ...
		 try{
			 ...
			 sb.append(" DELETE T_FAMILY_HISTORY					 ");
			 sb.append(" WHERE APPLY_CODE = ? 					 ");
