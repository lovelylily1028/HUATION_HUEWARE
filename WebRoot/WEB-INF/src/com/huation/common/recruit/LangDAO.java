...
public class LangDAO extends AbstractDAO {
	...
	public LangDTO registResult(String applycode) throws DAOException{
		
		String apply_code="";
		String passwd="";
		StringBuffer sb = new StringBuffer();
		QueryStatement sql = new QueryStatement();
		sb.append(" SELECT APPLY_CODE FROM T_LANG_HISTORY ");
	...
	public int[] addHistory(String[] commNos) throws Exception{
		...
		try{
			...
			for(int i=0; commNos != null && i<commNos.length; i++){ 
				...
				*/
				sb.append(" INSERT T_LANG_HISTORY  (APPLY_CODE,HISTORY_CODE,LANGUAGE,EXAM_TYPE,GRADE,TAKE_YMD,ISSUE_ORG) 	\n");
	...
	public ArrayList historyListView( String apply_code) throws DAOException{
		...
		LangDTO langDto  =null;
		ArrayList<LangDTO> arrlist = new ArrayList<LangDTO>();
		try{			
			sbSql.append(" SELECT APPLY_CODE,HISTORY_CODE,LANGUAGE,EXAM_TYPE,GRADE,TAKE_YMD,ISSUE_ORG ");
			sbSql.append(" FROM T_LANG_HISTORY ");
			...
			log.debug("historyListView["+sbSql.toString()+"]");	
			 ...
			 try{
				 ...
				 while(ds.next()){ 
					 langDto = new LangDTO();
					 langDto.setApply_code(ds.getString("APPLY_CODE"));
					 langDto.setHistory_code(ds.getString("HISTORY_CODE"));
					 langDto.setLanguage(ds.getString("LANGUAGE"));
					 langDto.setExam_type(ds.getString("EXAM_TYPE"));
					 langDto.setGrade(ds.getString("GRADE"));
					 langDto.setTake_ymd(ds.getString("TAKE_YMD"));
					 langDto.setIssue_org(ds.getString("ISSUE_ORG"));
	...
	public int deleteHistory(String apply_code) throws DAOException{
		 ...
		 try{
			 ...
			 sb.append(" DELETE T_LANG_HISTORY					 ");
			 sb.append(" WHERE APPLY_CODE = ? 					 ");
