...
public class HistoryDAO extends AbstractDAO {
	...
	public String getHistoryCntNext() throws DAOException{
		...
		sql.setSql("SELECT (ISNULL(MAX(HISTORY_CODE),0)+1) H_CNT FROM T_HISTORY ");
	...
	public HistoryDTO registResult(String applycode) throws DAOException{
		
		String apply_code="";
		String passwd="";
		StringBuffer sb = new StringBuffer();
		QueryStatement sql = new QueryStatement();
		sb.append(" SELECT APPLY_CODE FROM T_HISTORY ");
		...
		DataSet ds = null;
		HistoryDTO resultDto=new HistoryDTO();
	...
	public int[] addHistory(String[] commNos) throws Exception{
		...
		try{
			...
			for(int i=0; commNos != null && i<commNos.length; i++){ 
				...
				*/
				sb.append(" INSERT T_HISTORY  (APPLY_CODE,HISTORY_CODE,SCHOOL_NM,START_YM,GRAD_YM,MAJOR,CREDIT,GRAD_GB) 	\n");
	...
	public ArrayList historyListView( String apply_code) throws DAOException{
		...
		ArrayList<HistoryDTO> arrlist = new ArrayList<HistoryDTO>();
		try{			
			sbSql.append(" SELECT APPLY_CODE,HISTORY_CODE,SCHOOL_NM,START_YM,GRAD_YM,MAJOR,CREDIT,GRAD_GB ");
			sbSql.append(" FROM T_HISTORY ");
			...
			log.debug("historyListView["+sbSql.toString()+"]");	
			 ...
			 try{
				 ...
				 while(ds.next()){ 
					 ...
					 historyDto.setApply_code(ds.getString("APPLY_CODE"));
					 historyDto.setHistory_code(ds.getString("HISTORY_CODE"));
					 historyDto.setSchool_nm(ds.getString("SCHOOL_NM"));
					 historyDto.setStart_ym(ds.getString("START_YM"));
					 historyDto.setGrad_ym(ds.getString("GRAD_YM"));
					 historyDto.setMajor(ds.getString("MAJOR"));
					 historyDto.setCredit(ds.getString("CREDIT"));
					 historyDto.setGrad_gb(ds.getString("GRAD_GB"));
					 
					 arrlist.add(historyDto);
				 }
			 }catch(Exception e){
			      e.printStackTrace();
			      log.error(e.getMessage());
