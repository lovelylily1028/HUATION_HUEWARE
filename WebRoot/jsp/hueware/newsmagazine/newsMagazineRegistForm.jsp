...
function goSave(){
	var frm = document.newsMagazineRegist; 
...
/* 	if(fsize > 524288000 ){   //파일용량 체크
	...
	*/     
	...
	function cancle(){
		...
		var frm = document.newsMagazineRegist;
		frm.action='<%= request.getContextPath()%>/B_NewsMagazine.do?cmd=newsMagazinePageList';
		frm.submit();
...
<!-- //header -->
	...
	<div id="container">
		<div id="content">
			...
			<h3><span>N</span>ews &amp; Magazine등록</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
			<div class="con" id="excelBody">
				...
				<!-- 등록 -->
			    <form name="newsMagazineRegist" method="post" action="<%= request.getContextPath()%>/B_NewsMagazine.do?cmd=newsMagazineRegist" enctype="multipart/form-data">
			     <input type = "hidden" name = "curPage" value="<%=curPage%>"/>
				 <input type = "hidden" name = "user_id" value="<%=dtoUser.getUserId()%>"/>

				<fieldset>
					<legend>News &amp; Magazine등록</legend>
					<table class="tbl_type" summary="News &amp; Magazine등록(게시자, 첨부파일, 제목, 내용)">
						<colgroup>
							<col width="20%" />
							<col width="*" />
						</colgroup>
        				...
        				<input type="hidden" name="NewsFileNm" value=""></input>
						
						<tr>
							<th><label for="">첨부파일</label></th>
							<td><div class="fileUp"><input type="text" class="text" id="file1" title="첨부파일" style="width:561px;" /><a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span></a><input type="file" name="NewsFile" id="upload1" /></div><span class="guide_txt">&nbsp;&nbsp;* 첨부가능 파일유형 : 모든 파일 / 첨부가능 용량 : 최대 500M</span></td>
						</tr>
