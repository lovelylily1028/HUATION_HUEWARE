...
<!--
	...
	function goSelect(publicno,e_comp_nm,title){
		...
		var project_name;						//넘겨줄 변수 셋팅(프로젝트 명 변수 셋팅 후 openner로 넘겨주기.)
		var pub_project_name=eval('opener.document.<%=sForm%>.Pub_ProjectName');
		
		public_no.value=publicno;				//견적번호 변수에 값 셋팅 후 opener로 넘겨주기.
		purchase_name.value=e_comp_nm;			//업체명(발주사) 변수에 값 셋팅 후 opener로 넘겨주기.
		pub_project_name.value=title;			//견적번호에 대한 프로젝트명 값 셋팅 후 opener로 넘겨주기.
		
		pub_project_name.title=title;		//프로젝트명 input box readOnly 셋팅.
		
		var checkYN = opener.document.getElementById("readOnly").value;	//부모창 필드 값 가져오기.(readOnly 체크박스 판단 여부 값.)
		
		//프로젝트명 계약명으로 셋팅 되 있을 경우. 
		if(checkYN == 'Y'){
			
			/**프로젝트명 변경 안함.*/
			
		//프로젝트명 계약명으로 셋팅 안되 있을 경우.
		}else{
			project_name=eval('opener.document.<%=sForm%>.ProjectName');
			project_name.value=title;				//프로젝트명 변수에 값 셋팅 후 openner로 넘겨주기.
		}
