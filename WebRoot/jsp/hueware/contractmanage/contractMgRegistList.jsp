...
function init_contractMg() {
	...
	formObj=document.contractMgRegistListForm; //form객체세팅
...
function goDetail(contractcode){
	 ...
	 formObj.target ='_self';
	 formObj.action = "<%= request.getContextPath()%>/B_ContractManage.do?cmd=contractRegistMgView";
...
<div id="content">
  ...
  <%=ld.getPageScript("contractMgRegistListForm", "curPage", "goPage")%>
  <form method="post" name="contractMgRegistListForm" action="<%=request.getContextPath()%>/B_ContractManage.do?cmd=contractMgRegistList" class="search">
	...
	<a href="javascript:goRegist();" class="btn_type01 md0"><span>등록하기</span></a></div>
	<!-- //Top 버튼영역 -->
	</div>
	<!-- //컨텐츠 상단 영역 -->
    ...
    <tbody>    
        <tr>
          <td><a href="javascript:goDetail('<%=ds.getString("ContractCode")%>');"><%=ds.getString("ContractCode")%></a><a href="javascript:goInvoiceList('<%=ds.getString("ContractCode")%>','<%=ds.getString("ContractName")%>');" class="btn_type03"><span>발행내역</span></a></td>
