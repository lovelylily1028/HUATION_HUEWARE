...
function inputCheckSpecial(){
	...
	function cancle(){
		...
		if ("P" == frm.listType.value) {
			frm.action='<%= request.getContextPath()%>/N_Project.do?cmd=projectPageList_Non';	
		} else {
			frm.action='<%= request.getContextPath()%>/N_Project.do?cmd=projectPageListNonAll';
		}
...
<!-- //필수입력사항텍스트 -->
    ...
    <form name="projectRegist" method="post" action="<%= request.getContextPath()%>/N_Project.do?cmd=projectRegist_Non"  enctype="multipart/form-data">
	...
	<fieldset>
	<legend>비정기점검등록</legend>
      <table class="tbl_type" summary="비정기점검등록(고객사, 사유, 시작일자, 종료일자, 작성자, 담당자, 요청자, 수행장소, 수행내용, 특이사항)">
        <caption>비정기점검사이트 등록</caption>
       <colgroup>
		<col width="20%" />
		<col width="*" />
		</colgroup>
       ...
       <tr>
        ...
        <tr>
         <th><label for=""><span class="must_ico"><img src="<%= request.getContextPath()%>/images/common/must_icon_01.gif" alt="필수입력" /></span>종료일자</label></th>
          <td><span class="ico_calendar"><input id="calendarData2" name="EndDateTime" class="text" style="width:100px;" value="<%=strDate%>"onkeyup="javascript:datepickerInputText2(); dateProgressStatus();" onchange="javascript:dateProgressStatus();"/></span>&nbsp;&nbsp;종료시간&nbsp;&nbsp;<select name="End_Hour" id="End_Hour" title="시간 선택">
							<option value='00'>00시</option>
							<option value='01'>01시</option>
							<option value='02'>02시</option>
							<option value='03'>03시</option>
        ...
        <tr>
     	...
     	<tr>
          ...
          <td><div class="fileUp"><input type="text" class="text" id="file1" title="작업관련문서" style="width:494px;" /><a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span></a><input type="file" name="ReportFile" id="upload1" /></div><span class="guide_txt">&nbsp;&nbsp;* 첨부가능 파일유형 : PDF, GIF, JPG, TIF, BMP / 첨부가능 용량 : 최대 10M</span></td>
        </tr>
        </tbody>
      </table>
      </fieldset>
