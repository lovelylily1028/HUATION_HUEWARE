...
<script type="text/javascript">
	...
	function inits() {
		...
		formObj=document.huebookRePageList; //form객체세팅
	...
	function searchInit(){
		 ...
		 formObj.searchGb.value='<%=searchGb%>'; //조회옵션 구분 셀렉트값
	...
	function searchChk(){
		...
		if( formObj.searchGb[0].selected==true){
			formObj.searchtxt.disabled=false; //텍스트창 활성화 비활성화
  ...
  <form method="post" name="huebookRePageList" action="<%=request.getContextPath()%>/B_HueBookManageRe.do?cmd=hueBookRePageList" class="search">
	...
	<legend>검색</legend>
   		 ...
   		 <select name="searchGb" onchange="searchChk()" id="" class="">
	   		 <option checked value="1">도서명</option>
    ...
    <!--//search -->
    <!-- Top 버튼영역 -->
	<div class="Tbtn_areaR"><a href="javascript:goExcel();" class="btn_type01 btn_type01_gray"><span>내려받기</span></a><a href="javascript:goRegist();" class="btn_type01 md0"><span>신청하기</span></a></div>
	<!-- //Top 버튼영역 -->    
	</div>
      <table class="tbl_type tbl_type_list" summary="도서신청리스트(진행상태, 번호, 도서명, 출판사, 저자, 신청자, 신청가격, 결재일자, 구매가격 , 구매일자, 구매처, 승인/반려사유, 작성일시)">
...
<!-- //paginate -->
      ...
      <div class="Bbtn_areaR">
        <a href="javascript:goRegist();" class="btn_type01 md0"><span>신청하기</span></a>
      </div>
    </div>
    <!-- //con -->
