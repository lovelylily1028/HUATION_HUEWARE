...
<script type="text/javascript">
	...
	function goDetail(pjseq){
		 ...
		 formObj.target ='_self';
		 formObj.action = "<%= request.getContextPath()%>/B_ProjectCodeManage.do?cmd=projectCodeMgProgressView";
	...
	function goExcel() {  
		 ...
		 formObj.action = "<%=request.getContextPath()%>/B_ProjectCodeManage.do?cmd=projectCodeMgPageList_Progress_Excel";	
		 formObj.submit();
		 formObj.action = "<%=request.getContextPath()%>/B_ProjectCodeManage.do?cmd=projectCodeMgPageList_Progress";	
	}
...
<div id="content">
    <!-- title -->
    <div class="content_navi">프로젝트지원 &gt; 프로젝트진행관리</div>
	<h3><span>프</span>로젝트진행관리</h3>
    <!-- //title -->  
...
%>
  ...
  <form method="post" name="projectMgPageListSearchForm" action="<%=request.getContextPath()%>/B_ProjectCodeManage.do?cmd=projectCodeMgPageList_Progress" class="search">
    ...
    <!--//search -->
      ...
      <table class="tbl_type tbl_type_list" summary="프로젝트진행관리리스트(No., 진행, 프로젝트진행요청서, 검수증빙문서, 최종산출물, 프로젝트코드, 프로젝트구분, 프로젝트명, 고객사, 발주사, 담당영업, 담당PM, 프로젝트시작일, 프로젝트종료일, 프로젝트진행기간, 프로젝트잔여일, 등록자, 등록일시)">
        <caption>프로젝트 진행관리 리스트</caption>
        ...
        <%
                if (ld.getItemCount() > 0) {
                    ...
                    while (ds.next()) {
                    	...
                    	/*2014-03-06 대표님 요청으로 잔여일이 남지 않았을때 0일이 아닌 -일수로 처리 하기 위해 사용안함.
                    	
                    	String ProgressNowDate = ds.getString("ProgressNowDate");	//남은기간 가져오기.
                        String ProgressDateSubStr = ProgressNowDate.substring(0,1); //남은기간 지나서 -로 표기되는 일수 0으로 처리함.
                    	String ProgressDateStr = "";	//데이타 담는 변수.
                    	if(ProgressDateSubStr.equals("-")){
                    		ProgressDateStr = "0";
                    	}else{
                    		ProgressDateStr = ProgressNowDate;
                    	}
                    	*/
        %>
        <tr>
...
//                    String serverUrl = "https://" + request.getServerName() + ":" + request.getServerPort()+request.getContextPath();
		...
		<!--최종산출물 없을때.  -->
          ...
          <td><%=ds.getString("ProgressNowDate") %>일</td>
