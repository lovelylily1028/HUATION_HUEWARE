...
<script type="text/javascript">
	...
	function goRegist(){
		...
		formObj.action = "<%= request.getContextPath()%>/N_Project_H.do?cmd=projectRegistForm_Non_H";
		formObj.submit();
	}
	...
	function goDetail(Seq){
		 ...
		 formObj.target ='_self';
		 formObj.action = "<%= request.getContextPath()%>/N_Project_H.do?cmd=projectView_Non_H";
	...
	//Excel Export
	function goExcel() {
		 formObj.action = "<%=request.getContextPath()%>/N_Project_H.do?cmd=projectListExcel_Non_H";	
		 formObj.submit();
		 formObj.action = "<%=request.getContextPath()%>/N_Project_H.do?cmd=projectPageList_Non_H";	
	}
...
%>
  ...
  <form method="post" name=projectPageListForm action="<%=request.getContextPath()%>/N_Project_H.do?cmd=projectPageList_Non_H" class="search">
    ...
    <div class="Tbtn_areaR"><a href="javascript:goExcel();" class="btn_type01 btn_type01_gray"><span>내려받기</span></a><a href="javascript:goRegist();" class="btn_type01 md0"><span>등록하기</span></a></div>
      ...
      <table class="tbl_type tbl_type_list" summary="비정기점검리스트(한솔팩스)(고객사, 사유, 시작일자, 시작시간, 종료일자, 종료시간, 소요시간(Hour), 담당자, 요청자, 수행장소, 수행내용, 특이사항, 작성일시, 정기정검보고서)">
