...
<script type="text/javascript">
	...
	//초기세팅
	function init_formfile() {
		
		formObj=document.filePageListForm; //form객체세팅
	...
	function goRegist(){
		...
		formObj.action = "<%= request.getContextPath()%>/B_FormFile.do?cmd=formFileRegistForm";
		formObj.submit();
	}
...
<!-- //header -->
	...
	<div id="container">
		<div id="content">
			...
			<h3><span>서</span>식파일</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
			<div class="con">
				<!-- 컨텐츠 상단 영역 -->
			  ...
			  <%=ld.getPageScript("filePageListForm", "curPage", "goPage")%>
			  <form method="post" name=filePageListForm action="<%=request.getContextPath()%>/B_FormFile.do?cmd=formFilePageList" class="search">
				...
				<!-- //컨텐츠 상단 영역 -->
				<!-- 리스트 -->
				<table class="tbl_type tbl_type_list" summary="서식파일리스트(첨부파일, 분류, 등록자, 서식작성자, 등록일, 등록시간, 서식명, 조회수)">
					<colgroup>
						<col width="5%" />
						<col width="12%" />
						<col width="8%" />
     ...
     <tbody>   
        <tr>
          <td>
          		<%
                   String FormFile=ds.getString("FormFile");
          		   String FormFileNm=ds.getString("FormFileNm");
                   ...
                   //String serverUrl = "http://" + request.getServerName() + ":8088" +request.getContextPath();
                    ...
                    int c_index=FormFile.lastIndexOf("/");
                  	
                    String formfilename=FormFile.substring(c_index+1);
              
                    String formFilepath=""; //파일경로 읽어오기
                   
                    	
          
                    if(!FormFile.equals("")){
                    	formFilepath=FormFile.substring(0,c_index); //파일경로 읽어오기
                    	...
                    	String spStrReplace = "";
                    	if(FormFileNm.indexOf("&") != -1){
                    		spStrReplace=  FormFileNm.replace("&", "[replaceStr]");
                    	System.out.println("spStrReplace:"+spStrReplace);
                    	}else{
                    		spStrReplace =  FormFileNm;	
                    	System.out.println("spStrReplace:"+spStrReplace) ;
                    	}
                 %>
           
            <a href="<%=serverUrl %>/fileDownServlet?rFileName=<%=spStrReplace%>&sFileName=<%=formfilename%>&filePath=<%=formFilepath%>" ><img src="<%= request.getContextPath()%>/images/ico_down01.gif" width="16" height="16" align="absmiddle" title="첨부파일" /></a>
