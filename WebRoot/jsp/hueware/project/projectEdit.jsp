...
<%@ page import ="com.huation.common.project.ProjectDTO"%>
<%@ page import ="java.text.SimpleDateFormat" %>
<%

	Map model = (Map)request.getAttribute("MODEL");
	
	ProjectDTO pjDto = (ProjectDTO)model.get("pjDto");
...
function fileCheckNm(szValue){
	...
	if(confirm("수정 하시겠습니까?")){
		frm.action='<%= request.getContextPath()%>/B_Project.do?cmd=projectEdit';
...
function goDelete(){
	...
	if(confirm("삭제 하시겠습니까?")){
		frm.action='<%= request.getContextPath()%>/B_Project.do?cmd=projectDelete';
		frm.submit();
	}
...
function goList(){
	...
	var frm = document.projectView;
	frm.action='<%= request.getContextPath()%>/B_Project.do?cmd=projectPageList2';
	frm.submit();
...
<h3><span>정</span>기점검상세정보</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
  <!-- //title -->
<%--   <!-- calendar -->
  ...
  <!-- con -->
    ...
    <form name="projectView" method="post" action="<%= request.getContextPath()%>/B_Project.do?cmd=projectEdit" enctype="multipart/form-data">
     ...
     <input type = "hidden" name = "user_id" value="<%=pjDto.getChargeID() %>">
      <input type = "hidden" name = "ChargeID" value="<%=pjDto.getChargeID() %>">
       <input type="hidden" name="FrDate" value="<%=pjDto.getFrDate() %>">
       <input type="hidden" name="ToDate" value="<%=pjDto.getToDate() %>">
     ...
     <input type="hidden" id="listType" name="listType" value="<%=listType %>">  
      ...
      <fieldset>
		<legend>정기점검상세정보</legend>
      <table class="tbl_type" summary="정기점검상세정보(고객사, 시작일자, 종료일자, 대상월, 작성자, 담당자, 고객사담당자, 수행장소, 수행내용, 특이사항, 정기점검보고서)">
        ...
        <tr>
          ...
          <td>
            <span class="ico_calendar"><input id="calendarData1" name="StartDateTime" class="text" style="width:100px;" value="<%=StartDateTime%>" onkeyup="javascript:datepickerInputText1();  dateProgressStatus();" onchange="javascript:dateProgressStatus();"/></span>&nbsp;&nbsp;시작시간&nbsp;&nbsp;<%
        ...
        <tr>
          ...
          <td>
          	...
          	<span class="ico_calendar"><input id="calendarData2" name="EndDateTime" class="text" style="width:100px;" value="<%=EndDateTime%>"onkeyup="javascript:datepickerInputText2(); dateProgressStatus();" onchange="javascript:dateProgressStatus();"/></span>&nbsp;&nbsp;종료시간&nbsp;&nbsp;<%
       ...
       <tr>
          ...
          <td><input type="text" name="" class="text dis" style="width:200px;" readOnly value="<%=pjDto.getCreateUserName()%>"/></td>
        </tr>
        <tr>
          ...
          <td><input type="text" name="user_nm" class="text" style="width:200px" readOnly value="<%=pjDto.getChargeName() %>" onClick="javascript:popSales();"/><a href="javascript:popSales();" class="btn_type03"><span>사원조회</span></a></td>
        </tr>
         <tr>
          ...
          <td><input type="text" name="CustChargeNm"  value="<%=pjDto.getCustChargeNm() %>" onkeyup="inputCheckSpecial()" class="text" style="width:200px;" maxlength="10"  /></td>
        </tr>
         <tr>
          ...
          <td><input type="text" name="WorkSite" value="<%=pjDto.getWorkSite() %>" class="text" style="width:916px;" maxlength="50"  /></td>
        </tr>
         <tr>
          ...
          <td><input type="text" name="WorkContents" value="<%=pjDto.getWorkContents() %>" class="text" style="width:916px;" maxlength="50"  /></td>
        </tr>
         <tr>
          <th><label for="">특이사항</label></th>
          <td><textarea name="IssueReport" class="text" style="width:916px;height:41px;" dispName="특이사항" onKeyUp="js_Str_ChkSub('500', this)"><%=pjDto.getIssueReport()%></textarea></td>
        </tr>
...
// 				String serverUrl = "https://" + request.getServerName() + ":" + request.getServerPort()+request.getContextPath();
     	...
     	<tr>
          ...
          <td>
          	<div class="fileUp"><input type="text" class="text" id="file1" title="정기점검보고서" style="width:494px;" value="<%=pjDto.getReportFile() %>" />
          		<a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span>
          		<input type="file" name="ReportFile" id="upload1" />
          		</a>
         	</div><br />
            ...
            <input type="hidden" name="FileNm" value="<%=pjDto.getFileNm()%>" />
   ...
   <!-- button -->
    <div class="Bbtn_areaC">
    	...
    	<a href="javascript:goDelete();" class="btn_type02 btn_type02_gray"><span>삭제</span>
