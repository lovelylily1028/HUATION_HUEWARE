...
<script type="text/javascript">
	...
	//초기세팅
	function init_dispNotify() {
		
		formObj=document.dispPageListForm; //form객체세팅
	...
	function goRegist(){
		...
		formObj.action = "<%= request.getContextPath()%>/B_DispNotify.do?cmd=dispNotifyRegistForm";
		formObj.submit();
	}
...
<!-- //header -->
	...
	<div id="container">
		<div id="content">
			...
			<h3><span>전</span>사공지</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
			<div class="con">
				<!-- 컨텐츠 상단 영역 -->
		...
		%>
		  <%=ld.getPageScript("dispPageListForm", "curPage", "goPage")%>
		  <form method="post" name=dispPageListForm action="<%=request.getContextPath()%>/B_DispNotify.do?cmd=dispNotifyPageList" class="search">
		     <input type="hidden" name="curPage" value="<%=curPage%>"/>
		     <input type="hidden" name="Seq" value=""/>
		     <input type="hidden" name="ReadCount" value=""></input>
				...
				<!-- //컨텐츠 상단 영역 -->
 				...
 				<!-- 리스트 -->
				<table class="tbl_type tbl_type_list" summary="전사공지리스트(첨부파일, 게시자, 등록일, 등록시간, 제목, 조회수)">
					<colgroup>
						<col width="5%" />
						<col width="8%" />
						<col width="8%" />
     ...
     <tbody>   
        <tr>
          <td>
          <%
                   String NotifyFile=ds.getString("NotifyFile");
          		   String NotifyFileNm=ds.getString("NotifyFileNm");
				   ...
				   String serverUrl = "" + request.getContextPath();
                    ...
                    int c_index=NotifyFile.lastIndexOf("/");
                  	
                    String notifyfilename=NotifyFile.substring(c_index+1);
              
                    String disNotifypath=""; //파일경로 읽어오기
                   
                    	
          
                    if(!NotifyFile.equals("")){
                    	disNotifypath=NotifyFile.substring(0,c_index); //파일경로 읽어오기
                    	...
                    	String spStrReplace = "";
                    	if(NotifyFileNm.indexOf("&") != -1){
                    		spStrReplace=  NotifyFileNm.replace("&", "[replaceStr]");
                    	System.out.println("spStrReplace:"+spStrReplace);
                    	}else{
                    		spStrReplace =  NotifyFileNm;	
                    	System.out.println("spStrReplace:"+spStrReplace) ;
                    	}
                 %>
           
            <a href="<%=serverUrl %>/fileDownServlet?rFileName=<%=spStrReplace%>&sFileName=<%=notifyfilename%>&filePath=<%=disNotifypath%>" ><img src="<%= request.getContextPath()%>/images/ico_down.gif" width="16" height="16" align="absmiddle" title="첨부파일"></a>
            	<%
                     }
