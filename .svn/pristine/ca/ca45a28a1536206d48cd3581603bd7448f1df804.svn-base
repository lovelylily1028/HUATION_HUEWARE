...
<%
	...
	NonProjectDTO npjDto = (NonProjectDTO)model.get("npjDto");
	...
	String listType =  (String)model.get("listType");
		...
		//프로젝트 시작예정일 초기 셋팅 값.
			...
			if(npjDto.getStartDate().equals("")){
				System.out.println("111");
				StartDateTime = npjDto.getStartDate();
				StartDateTime = new SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date());
			}else{
			    ...
			    strY_Val1 = StartDateTime.substring(0,4);
			    strM_Val1 = StartDateTime.substring(4,6);
			    strD_Val1 = StartDateTime.substring(6,8);
				
			    StartDateTime = strY_Val1 + '-' + strM_Val1  + '-' + strD_Val1;
			}
			
			//프로젝트 종료예정일 초기 셋팅 값.
			String EndDateTime = "";
			
			if(npjDto.getEndDateTime().equals("")){
				EndDateTime = npjDto.getEndDate();
				EndDateTime = new SimpleDateFormat("yyyy-MM-dd").format(new java.util.Date());
			}else{
				EndDateTime = npjDto.getEndDate();
				String strY_Val1;										//년도 값 자르면서 arr형태로 변환되어 String으로 형변환하여 담기.(벨류데이션체크를 위해)
				String strM_Val1;										//월자 값 자르면서 arr형태로 변환되어 String으로 형변환하여 담기.(벨류데이션체크를 위해)
			    ...
			    strY_Val1 = EndDateTime.substring(0,4);
			    strM_Val1 = EndDateTime.substring(4,6);
			    strD_Val1 = EndDateTime.substring(6,8);
				
			    EndDateTime = strY_Val1 + '-' + strM_Val1  + '-' + strD_Val1;
			}
...
function fileCheckNm(szValue){
	...
	if(confirm("수정 하시겠습니까?")){
		frm.action='<%= request.getContextPath()%>/N_Project.do?cmd=projectEdit_Non';
		if(frm.CompanyCode.value == ""){
			alert("비정기점검 대상 사이트를 선택하세요");
			return;
		}
		if(frm.CheckReason.value == ""){
			alert("사유를 선택하세요");
			return;
		}
...
function inputCheckSpecial(){
	var strobj = document.projectView.RequestNm;
	re =/[.,~!@\#$%^&*\()\-=+_]/;
...
//달력 텍스트 입력창 첫번째.
 function datepickerInputText1(){
	  var frm = document.projectView;	//폼생성
...
//달력 텍스트 입력창 두번째.
 ...
 {
	var frm = document.projectView;		//form
	...
	*/
     ...
     if(to_dt.getTime() < from_dt.getTime()){
			...
			frm.EndDateTime.value='<%=EndDateTime%>';	
			frm.StartDateTime.value='<%=StartDateTime%>';
...
function popSales(){
	var a = window.open("<%= request.getContextPath()%>/B_Common.do?cmd=searchUser&sForm=projectView","","width=600,height=619,left=20,top=20,toolbar=no, location=no, directories=no, menubar=no, resizable=no, scrollbars=no, status=no");
}
//삭제
function goDelete(){
	...
	if(confirm("삭제 하시겠습니까?")){
		frm.action='<%= request.getContextPath()%>/N_Project.do?cmd=projectDelete_Non';
		frm.submit();
	}
...
function goList(){
	...
	var frm = document.projectView;
	frm.action='<%= request.getContextPath()%>/N_Project.do?cmd=projectPageList_Non2';
	frm.submit();
}
...
<div id="content">
  <!-- title -->
<div class="content_navi">프로젝트지원 &gt; 비정기점검</div>
<h3><span>비</span>정기점검상세정보</h3>
  <!-- //title -->
...
<!-- //필수입력사항텍스트 -->
				...
				<!-- //컨텐츠 상단 영역 -->
    <form name="projectView" method="post" action="<%= request.getContextPath()%>/N_Project.do?cmd=projectEdit_Non" enctype="multipart/form-data">
      <input type = "hidden" name = "curPage" value="<%=curPage%>">
      <input type = "hidden" name = "searchGb" value="<%=searchGb%>">
      <input type = "hidden" name = "Seq" value="<%=npjDto.getSeq()%>">
    <input type = "hidden" name = "user_id" value="<%=npjDto.getChargeID()%>">
      <input type = "hidden" name = "ChargeID" value="<%=npjDto.getChargeID() %>">
     ...
     <input type="hidden" name="FrDate" value="<%=npjDto.getFrDate() %>">
     <input type="hidden" name="ToDate" value="<%=npjDto.getToDate() %>">
     <input type = "hidden" id = "listType" name = "listType" value="<%=listType%>"/>
      
      <fieldset>
	<legend>비정기점검상세정보</legend>
     <table class="tbl_type" summary="비정기점검상세정보(고객사, 사유, 시작일자, 종료일자, 작성자, 담당자, 요청자, 수행장소, 수행내용, 특이사항)">
		<colgroup>
			<col width="20%" />
			<col width="*" />
		</colgroup>
		<tbody>
				...
				<td><%
							...
							codeParam.setSelected(npjDto.getCompanyCode()); 
			...
			<th><label for=""><span class="must_ico"><img src="<%= request.getContextPath()%>/images/common/must_icon_01.gif" alt="필수입력" /></span>사유</label></th>
				<td><%
							...
							codeParam.setStyleClass("td3");
							//codeParam.setFirst("전체");
							codeParam.setName("CheckReason");
							codeParam.setSelected(npjDto.getCheckReason()); 
							//codeParam.setEvent("javascript:poductSet();"); 
							out.print(CommonUtil.getCodeList(codeParam,"A05")); 
						%></td>		
				</tr>
        ...
        <tr>
          ...
          <td><span class="ico_calendar"><input id="calendarData1" name="StartDateTime" class="text" style="width:100px;" value="<%=StartDateTime%>" onkeyup="javascript:datepickerInputText1();  dateProgressStatus();" onchange="javascript:dateProgressStatus();"/></span><input type="hidden" class="text"  style="width:80px" value="<%=npjDto.getStartDate()%>" >&nbsp;&nbsp;시작시간&nbsp;&nbsp;<%
          			String StartTime=StringUtil.nvl(npjDto.getStartTime(),":");
					int indexs=StartTime.indexOf(":");
					String Start_Hour = StartTime.substring(0,indexs);
					String Start_Minute = StartTime.substring(indexs+1);
				%><select name="Start_Hour" id="Start_Hour" title="시간 선택" onchange="javascript:func_Select()">
        ...
        <tr>
          ...
          <td><span class="ico_calendar"><input id="calendarData2" name="EndDateTime" class="text" style="width:100px;" value="<%=EndDateTime%>" onkeyup="javascript:datepickerInputText2(); dateProgressStatus();" onchange="javascript:dateProgressStatus();"/></span><input type="hidden" class="text"  style="width:80px" value="<%=npjDto.getEndDate()%>">&nbsp;&nbsp;종료시간&nbsp;&nbsp;<%
					String EndTime=StringUtil.nvl(npjDto.getEndTime(),":");
					int indexx=EndTime.indexOf(":");
					String End_Hour = EndTime.substring(0,indexx);
					String End_Minute = EndTime.substring(indexx+1);
				%><select name="End_Hour" id="End_Hour" title="시간 선택">
							...
							<option value='22'>22시</option>
							<option value='23'>23시</option>
						</select> <select name="End_Minute" id="End_Minute" title="분 선택">
       ...
       <tr>
          ...
          <td><input type="text" name="" class="text dis" title="작성자" style="width:200px;" readOnly value="<%=npjDto.getCreateUserName()%>"/></td>
        </tr>
        <tr>
          ...
          <td><input type="text" name="user_nm" class="text"title="담당자*" style="width:200px;" readOnly value="<%=npjDto.getChargeName() %>" onClick="javascript:popSales();"/><a href="javascript:popSales();" class="btn_type03"><span>사원조회</span></a></td>
        </tr>
         <tr>
          <th><label for=""><span class="must_ico"><img src="<%=request.getContextPath() %>/images/common/must_icon_01.gif" alt="필수입력" /></span>요청자</label></th>
          <td><input type="text" name="RequestNm"  value="<%=npjDto.getRequestNm() %>" onkeyup="inputCheckSpecial()" class="text" title="요청자" style="width:200px;" maxlength="10"  /></td>
        </tr>
         <tr>
          ...
          <td><input type="text" name="WorkSite" value="<%=npjDto.getWorkSite() %>" class="text" title="수행장소" style="width:916px;" maxlength="50"  /></td>
        </tr>
         <tr>
          ...
          <td><input type="text" name="WorkContents" value="<%=npjDto.getWorkContents() %>" class="text" title="수행내용" style="width:916px;" maxlength="50"  /></td>
        </tr>
         <tr>
          <th><label for="">특이사항</label></th>
          <td><textarea name="IssueReport" class="text" title="특이사항" style="width:916px;height:41px;" dispName="특이사항 " onKeyUp="js_Str_ChkSub('500', this)"><%=npjDto.getIssueReport()%></textarea></td>
        </tr>
          <%
			   String ReportFile=npjDto.getReportFile();
...
// 				String serverUrl = "https://" + request.getServerName() + ":" + request.getServerPort()+request.getContextPath();
				...
				int a_index=ReportFile.lastIndexOf("/");
				
				String reportfilename=ReportFile.substring(a_index+1);
				
			%>
     	<tr>
          <th><label for="">작업관련문서</label></th>
         
          <td><div class="fileUp"><input type="text" class="text" id="file1" title="정기점검보고서" style="width:852px;" value="<%=npjDto.getReportFile() %>" /><a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span></a><input type="file" name="ReportFile" id="upload1" /></div><br />
            <% if(!npjDto.getReportFile().equals("")){ %>
            <!--  -->
             <% } %>
            <input type="hidden" name="p_report_file" value="<%=npjDto.getReportFile()%>" />
            <input type="hidden" name="FileNm" value="<%=npjDto.getFileNm()%>" />
            <span class="guide_txt br">* 첨부가능 파일유형 : PDF, GIF, JPG, TIF, BMP / 첨부가능 용량 : 최대 10M<br />
               <%	
               	if(!npjDto.getReportFile().equals("")){
               		
               %>
               <strong>* 첨부파일명 : <%=npjDto.getFileNm() %>입니다.</strong></span>
               ...
               <strong>* 첨부된 파일이 없습니다.</strong></span>
               	<%
				}
   ...
   <!-- button -->
    <div class="Bbtn_areaC">
    	<a href="javascript:goSave();" class="btn_type02"><span>수정</span></a>
...
<%= comDao.getMenuAuth(menulist,"24") %>
