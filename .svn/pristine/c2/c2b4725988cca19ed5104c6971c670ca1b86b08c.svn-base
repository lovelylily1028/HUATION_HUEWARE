...
function goSave(){
	var frm = document.formFileRegist; 
...
/* 	function isFile( obj ) {
	...
	function popSales(){
		var a = window.open("<%= request.getContextPath()%>/B_Common.do?cmd=searchUser&sForm=formFileRegist","","width=600,height=619,left=20,top=20,toolbar=no, location=no, directories=no, menubar=no, resizable=no, scrollbars=no, status=no");
}
	
	function cancle(){
		
		var frm = document.formFileRegist;
		frm.action='<%= request.getContextPath()%>/B_FormFile.do?cmd=formFilePageList';
		frm.submit();
...
<!-- //header -->
	...
	<!-- container -->
	<div id="container">
		<div id="content" id="excelBody">
			<div class="content_navi">게시판 &gt; 서식파일</div>
			<h3><span>서</span>식파일등록</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
			<div class="con">
				<!-- 컨텐츠 상단 영역 -->
				...
				<!-- 등록 -->
			   <form name="formFileRegist" method="post" action="<%= request.getContextPath()%>/B_FormFile.do?cmd=formFileRegist" enctype="multipart/form-data">
			    <input type = "hidden" name = "curPage" value="<%=curPage%>"/>
			    <input type = "hidden" name = "user_id" value="<%=dtoUser.getUserId()%>"/>
				 <input type ="hidden" name="WriteFormUser" value=""></input>

				<fieldset>
					<legend>서식파일등록</legend>
					
					<table class="tbl_type" summary="서식파일등록(등록자, 서식작성자, 서식분류, 첨부파일, 제목, 내용)">
						<colgroup>
							<col width="20%" />
							<col width="*" />
						</colgroup>
						...
						<tr>
							...
							<td><input type="text" id="" name="user_nm" class="text" title="서식작성자" style="width:200px;" readOnly value="<%=dtoUser.getUserNm()%>" onClick="javascript:popSales();"/><a href="javascript:popSales();" class="btn_type03"><span>사원조회</span></a></td>
						</tr>
        				...
        				<input type="hidden" name="FormFileNm" value=""></input>
						<tr>
							...
							<td><div class="fileUp"><input type="text" class="text" id="file1" title="첨부파일" style="width:561px;" /><a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span></a><input type="file" name="FormFile" id="upload1" /></div><span class="guide_txt">&nbsp;&nbsp;* 첨부가능 파일유형 : 모든 파일 / 첨부가능 용량 : 최대 500M</span></td>
						</tr>
