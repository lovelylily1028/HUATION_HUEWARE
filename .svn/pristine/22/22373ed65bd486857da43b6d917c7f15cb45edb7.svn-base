...
<div id="container">
	<div id="content">
		...
		<!-- title -->
		<div class="content_navi">영업지원 &gt; (구)세금계산서상세정보</div>
		<h3><span>(구)</span>세금계산서상세정보</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
		<!-- title -->
  ...
  <!-- //calendar -->
    ...
    <form name="invoiceVeiw" method="post" action="<%= request.getContextPath()%>/B_Invoice.do?cmd=invoiceEdit">
 	...
 	<fieldset>
		<legend>(구)세금계산서상세정보</legend>
		<table class="tbl_type" summary="(구)세금계산서상세정보(견적서발행번호(발행번호, 제목), 계산서금액(공급가, 부가세, 합계), 구분(권, 호, 관리번호, 승인번호, 수신(수령)인), 발행일자, 전자세금계산서발행기관, 공급받는자(상호(법인명), 업태, 종목, 대표자명, 본점소재지), 입금예상일자, 입금예상계좌, 입금금액, 입금일자, 참고사항)">
		...
		<tbody>
		<tr class="last">
			<td><input type="text" id="" name="p_public_no" class="text" title="발행번호" style="width:188px;" readOnly onClick="javascript:popPublicNo();" value="<%=invoiceDto.getPublic_no()%>"/><a href="javascript:popPublicNo();" class="btn_type03"><span>발행번호조회</span></a></td>
										
			<th><label for="">제목</label></th>
			<td class="last"><input type="text" id="" name="title" class="text dis" title="제목" style="width:494px;" readOnly value="<%=invoiceDto.getTITLE()%>"/></td><!-- input 비활성화 class="dis" 추가 -->
		</tr>
		</tbody>
		</table>
			</td>
		...
		<tbody>
			<tr class="last">
				...
				<td><input type="hidden" name="supply_price" class="text"  style="width:80px" value="<%=invoiceDto.getSupply_price()%>" /><input type="hidden" name="vat" class="text"  style="width:80px" value="<%=invoiceDto.getVat()%>" /><input type="hidden" name="total_amt" class="text"  style="width:80px" value="" /><input type="text" id="" name="supply_price_view" class="text text_r" title="공급가" style="width:153px;" maxlength="18" value="<%=NumberUtil.getPriceFormat(invoiceDto.getSupply_price())%>"  onKeyUp = "saleCntCal('invoiceVeiw.supply_price')"/> 원</td>
														
				<th><label for="">부가세</label></th>
				<td><input type="text" id="" name="vat_view" class="text text_r" title="부가세" style="width:153px;" maxlength="18" value="<%=NumberUtil.getPriceFormat(invoiceDto.getVat())%>"  onKeyUp = "saleCntCal('invoiceVeiw.vat')"/> 원</td>
			...
			<th><label for=""><span class="must_ico"><img src="<%= request.getContextPath()%>/images/common/must_icon_01.gif" alt="필수입력" /></span>구분</label></th>
				...
				<tbody>
					<tr>
						...
						<td><input type="text" id="" name="gun" class="text" title="권" style="width:168px;" readOnly value="<%=invoiceDto.getGun()%>"/></td>
										
						<th><label for="">호</label></th>
						<td><input type="text" id="" name="ho" class="text" title="호" style="width:168px;" readOnly value="<%=invoiceDto.getHo()%>"/></td>
										
						<th><label for="">관리번호</label></th>
						<td class="last"><input type="text" name="manage_no" id="" class="text" title="관리번호" style="width:178px;" readOnly value="<%=invoiceDto.getManage_no()%>"/></td>
					</tr>

					<tr class="last">
						<th><label for="">승인번호</label></th>
						<td><input type="text" id="" name="approve_no" class="text" title="승인번호" style="width:168px;" maxlength="50"  value="<%=invoiceDto.getApprove_no()%>"/></td>
						
						<th><label for="">수신(수령)인</label></th>
						<td colspan="3" class="last"><input type="text" id="" name="receiver" class="text" title="수신(수령)인" style="width:200px;" maxlength="10"  value="<%=invoiceDto.getReceiver()%>"/></td>
					</tr>
				</tbody>
			</table>
		</td>
		...
		<tr>
			...
			<td><input type="text" id="" name="public_org" class="text" title="전자세금계산서발행기관" style="width:300px;" maxlength="25"  value="<%=invoiceDto.getPublic_org()%>"/></td>
		</tr>
					...
					<tbody>
						<tr>
							<th><label for="">상호(법인명)</label></th>
								<td><input type="text" id="" name="comp_nm" class="text" title="상호(법인명)" style="width:103px;" readOnly onClick="javascript:popComp();" value="<%=invoiceDto.getComp_nm()%>"/><a href="javascript:popComp();" class="btn_type03"><span>업체조회</span></a></td>
							
							<th><label for="">업태</label></th>
								<td><input type="text" id="" name="business" class="text" title="업태" style="width:168px;" value="<%=invoiceDto.getBusiness()%>"  readOnly/></td>
										
							<th><label for="">종목</label></th>
								<td class="last"><input type="text" id="" name="b_item" class="text" title="종목" style="width:178px;" value="<%=invoiceDto.getB_item()%>"  readOnly/></td>
						</tr>

   						<tr class="last">
							<th><label for="">대표자명</label></th>
							<td><input type="text" id="" name="owner_nm" class="text" title="대표자명" style="width:168px;" value="<%=invoiceDto.getOwner_nm()%>"  readOnly/></td>
										
							<th><label for="">본점소재지</label></th>
							<td colspan="3" class="last">
								<ul class="listD">
									<li class="first"><input type="text" id="" name="post" class="text" title="우편번호" style="width:80px;" value="<%=invoiceDto.getPost()%>"  readOnly/></li>
									
									<li>
										<input type="text" id="" name="address" class="text" title="기본주소" style="width:239px;" value="<%=invoiceDto.getAddress()%>"  readOnly /> 
										<input type="text" id="" name="addr_detail" class="text" title="상세주소" style="width:239px;" value="<%=invoiceDto.getAddr_detail()%>"  readOnly/>
									</li>
								</ul>
							</td>
						</tr>
		...
		<tr>
			<th><label for="">입금금액</label></th>
				<td><input type="hidden" name="deposit_amt" class="in_txt"  style="width:80px;" value="<%=invoiceDto.getDeposit_amt()%>" /><input type="text" id="" name="deposit_amt_view" class="text" title="입금금액" style="width:200px;" maxlength="18"  onKeyUp = "saleCntCal('invoiceVeiw.deposit_amt')" value="<%=NumberUtil.getPriceFormat(invoiceDto.getDeposit_amt())%>"/> 원<span class="guide_txt">&nbsp;&nbsp;* 입금금액 입력 시 입금일자는 필수 입력 항목입니다.</span></td>
		</tr>
		...
		<tr>
			<th><label for="">참고사항</label></th>
				<td><textarea id="" title="참고사항" name="reference" style="width:917px;height:41px;" onKeyUp="js_Str_ChkSub('500', this)"><%=invoiceDto.getReference()%></textarea></td>
		</tr>
		</tbody>
	</table>
	</fieldset>
