...
function inputCheckSpecial(){
	var strobj = document.projectRegist.RequestNm;
	re =/[.,~!@\#$%^&*\()\-=+_]/;
	  ...
	  function dateProgressStatus()
		...
		*/
	        ...
	        if(to_dt.getTime() < from_dt.getTime()){
				...
				//프로젝트 시작예정일이 더 클 시 전날 날짜로 초기 날짜 셋팅.
...
<h3><span>비</span>정기점검등록(한솔팩스)</h3>
  ...
  <!-- 컨텐츠 상단 영역 -->
    ...
    <form name="projectRegist" method="post" action="<%= request.getContextPath()%>/N_Project_H.do?cmd=projectRegist_Non_H" enctype="multipart/form-data">
	...
	<input type = "hidden" name = "pYear1" value=""/>
	 ...
	 <fieldset>
	 <legend>비정기점검등록(한솔팩스)</legend>
      <table class="tbl_type" summary="비정기점검등록(한솔팩스)(고객사, 사유, 시작일자, 종료일자, 작성자, 담당자, 요청자, 수행장소, 수행내용, 특이사항)">
       ...
       <tr>
        ...
        <tr>
         <th><label for=""><span class="must_ico"><img src="<%=request.getContextPath() %>/images/common/must_icon_01.gif" alt="필수입력" /></span>종료일자</label></th>
          <td >
          <span class="ico_calendar"><input id="calendarData2" name="EndDateTime" class="text" style="width:100px;" value="<%=strDate%>" onkeyup="javascript:datepickerInputText2(); dateProgressStatus();" onchange="javascript:dateProgressStatus();"/></span>&nbsp;&nbsp;종료시간&nbsp;&nbsp;<select name="End_Hour" id="End_Hour" title="시간 선택">
							<option value='00'>00시</option>
							<option value='01'>01시</option>
							<option value='02'>02시</option>
							<option value='03'>03시</option>
       ...
       <tr>
        ...
        <tr>
          ...
          <td><input type="text" name="user_nm" class="text" title="담당자*" style="width:200px" readOnly value="<%=dtoUser.getUserNm() %>" onClick="javascript:popSales();"/><a href="javascript:popSales();" class="btn_type03"><span>사원조회</span></a></td>
        </tr>
         <tr>
          ...
          <td><input type="text" name="RequestNm" value="" onkeyup="inputCheckSpecial()" class="text" title="요청자" style="width:200px;" maxlength="10"  /></td>
        </tr>
         <tr>
        ...
        <input type="hidden" name="FileNm" value=""></input>
     	<tr>
          ...
          <td><div class="fileUp"><input type="text" class="text" id="file1" title="정기점검보고서" style="width:494px;" /><a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span></a><input type="file" name="ReportFile" id="upload1" /></div><span class="guide_txt">&nbsp;&nbsp;* 첨부가능 파일유형 : PDF, GIF, JPG, TIF, BMP / 첨부가능 용량 : 최대 10M</span></td>
        </tr>
        </tbody>
      </table>
      </fieldset>
