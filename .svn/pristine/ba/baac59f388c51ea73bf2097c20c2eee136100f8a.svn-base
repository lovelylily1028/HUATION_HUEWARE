...
<script type="text/javascript">
	...
	function goExcel() {
		 ...
		 formObj.submit();
		 formObj.action = "<%=request.getContextPath()%>/N_Project.do?cmd=projectPageList_Non";	
	}
...
%>
 ...
 <!-- title -->
	...
	<h3><span>비</span>정기점검</h3>
    <!-- //title -->
    <div class="con">
    <div class="conTop_area">
  
  <%=ld.getPageScript("projectPageListForm", "curPage", "goPage")%>
  <form method="post" name=projectPageListForm action="<%=request.getContextPath()%>/N_Project.do?cmd=projectPageList_Non"class="search">
    ...
    <!-- search -->
      <fieldset>
		...
		<input type="hidden" name="CompanyCode" style="ime-mode:active;width:200px" class="search" maxlength="100" id="textfield_search2" value="<%=CompanyCode%>" onkeydown="if(event.keyCode == 13)  goSearch()" />
    	  	...
    	  	<li><input type="text" class="text" title="검색어" id="searchtxt" /></li>
  		    <li><a href="javascript:goSearch();" class="btn_type01"><span>검색</span></a></li>
     	 </ul>
      </fieldset>
      </form>
      <div class="Tbtn_areaR"><a href="javascript:goExcel();" class="btn_type01 btn_type01_gray"><span>내려받기</span></a><a href="javascript:goRegist();" value="등록하기"  class="btn_type01 md0"><span>등록하기</span></a></div>
    </div>
    <!--//search -->
