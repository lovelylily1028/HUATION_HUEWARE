...
<%@ page import ="com.huation.common.project_h.NonProject_H_DTO"%>
<%@ page import ="java.text.SimpleDateFormat" %>
<%

	Map model = (Map)request.getAttribute("MODEL");
	
	NonProject_H_DTO npjDto = (NonProject_H_DTO)model.get("npjDto");
...
function goDelete(){
	...
	if(confirm("삭제 하시겠습니까?")){
		frm.action='<%= request.getContextPath()%>/N_Project_H.do?cmd=projectDelete_Non_H';
		frm.submit();
	}
...
function goList(){
	...
	var frm = document.projectView;
	frm.action='<%= request.getContextPath()%>/N_Project_H.do?cmd=projectPageList_Non_H';
	frm.submit();
}
...
<div id="content">
  <!-- title -->
<div class="content_navi">프로젝트지원 &gt; 비정기점검(한솔팩스)</div>
<h3><span>비</span>정기점검상세정보(한솔팩스)</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
  <!-- //title -->
  <!-- con -->
  <div class="con">
 <!-- 컨텐츠 상단 영역 -->
...
<!-- //컨텐츠 상단 영역 -->
<!-- 등록 -->
    <form name="projectView" method="post" action="<%= request.getContextPath()%>/N_Project_H.do?cmd=projectEdit_Non_H" enctype="multipart/form-data">
	...
	<fieldset>
	<legend>비정기점검상세정보(한솔팩스)</legend>
	<table class="tbl_type" summary="비정기점검상세정보(한솔팩스)(고객사, 사유, 시작일자, 종료일자, 작성자, 담당자, 요청자, 수행장소, 수행내용, 특이사항)">
        <caption>비정기점검사이트 상세정보(한솔)</caption>
       <colgroup>
		<col width="20%" />
		<col width="*" />
		</colgroup>
		<tbody>
				...
				<td><%
							...
							out.print(CommonUtil.getCodeList(codeParam,"A04")); 
						%></td>
         		</tr>
				<tr>
        ...
        <tr>
          	...
          	<td><span class="ico_calendar"><input id="calendarData1" name="StartDateTime" class="text" style="width:100px;" value="<%=StartDateTime%>"onkeyup="javascript:datepickerInputText1();  dateProgressStatus();" onchange="javascript:dateProgressStatus();"/></span>&nbsp;&nbsp;시작시간&nbsp;&nbsp;<%
        ...
        <tr>
          ...
          <td><span class="ico_calendar"><input id="calendarData2" name="EndDateTime" class="text" style="width:100px;" value="<%=EndDateTime%>"onkeyup="javascript:datepickerInputText2(); dateProgressStatus();" onchange="javascript:dateProgressStatus();"/></span>&nbsp;&nbsp;종료시간&nbsp;&nbsp;<%
...
// 				String serverUrl = "https://" + request.getServerName() + ":" + request.getServerPort()+request.getContextPath();
     	...
     	<tr>
          ...
          <td><div class="fileUp"><input type="text" class="text" id="file1" title="정기점검보고서" style="width:852px;" value="<%=npjDto.getReportFile() %>" /><a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span></a><input type="file" name="ReportFile" id="upload1" /></div><br />
            ...
            <input type="hidden" name="FileNm" value="<%=npjDto.getFileNm()%>"></input>
            </span></a>
...
<%= comDao.getMenuAuth(menulist,"26") %>
