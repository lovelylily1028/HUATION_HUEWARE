...
function makeExcel(obj) {
	...
	function dtCheck(str, str2){
		...
		if (str2=="S"){
			alert("현재 판매준비 중입니다.\r"+yyyy+"년"+mm+"월"+dd+"일"+tt+"시 부터 이용하실 수 있습니다.");
		}else if (str2=="S"){
			alert("죄송합니다. 해당 상품은 판매가 중지되었습니다.");
		}
	}
	...
	function js_gift(){
		if( !chk_Goods('cateGoodsInfo', '선물 할') ) return;
		var pNm = "PLAYLIST_MYBOOK";
		jsOpenWindow("", "PLAYLIST_MYBOOK", "360", "210", "no", "center");
	...
	function check_player(str, goodsId, saleAge){
		...
		var data = str.split("|||");
		if(data[1]!=("pass")){
			alert("죄송합니다. 해당 상품은 재생이 불가능 합니다.");
			return;
		}
	...
	function check_Down(str, st1, saleAge){
		...
		if(data[0]==("Y")){
			...
			if(data[1]!=("pass")){
				alert("죄송합니다. 해당 상품은다운로드가 불가능 합니다.");
				return;
			}
	...
	function check_Shop(str, str1, saleAge){
		...
		if(data[0]==("Y")){
			...
			if(data[1]!=("pass")){
				alert("죄송합니다. 해당 상품은 구매가 불가능 합니다.");
				return;
			}
	...
	function check_Present(str, str1, saleAge){
		...
		var data = str.split("|||");
		if(data[0]==("Y")){
			alert("죄송합니다. 해당 상품은 선물이 불가능 합니다.");
				return;
		}else{
			if(data[1]!=("pass")){
				alert("죄송합니다. 해당 상품은 선물이 불가능 합니다.");
