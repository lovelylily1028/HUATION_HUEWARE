...
public class ProjectDAO extends AbstractDAO {
	...
	public ProjectDTO registResult(String applycode) throws DAOException{
		
		String apply_code="";
		String passwd="";
		StringBuffer sb = new StringBuffer();
		QueryStatement sql = new QueryStatement();
		sb.append(" SELECT APPLY_CODE FROM T_PROJECT ");
	 ...
	 * @throws DAOException
	 */
	public int[] addHistory(String[] commNos,String[] p_details) throws Exception{
		...
		try{
			...
			for(int i=0; commNos != null && i<commNos.length; i++){ 
				...
				*/
				sb.append(" INSERT T_PROJECT  (APPLY_CODE,HISTORY_CODE,PROJECT_NM,START_YMD,END_YMD,ORDER_ORG,PROC_ORG,ROLE,PROJECT_DETAIL) 	\n");
				sb.append(" VALUES('"+StringUtil.nvl(r_data[0],"")+"','"+StringUtil.nvl(history_code,"")+"','"+StringUtil.nvl(r_data[1],"")+"','"+StringUtil.nvl(r_data[2],"")+"','"+StringUtil.nvl(r_data[3],"")+"','"+StringUtil.nvl(r_data[4],"")+"','"+StringUtil.nvl(r_data[5],"")+"','"+StringUtil.nvl(r_data[6],"")+"','"+p_details[i]+"'); \n");
	...
	public ArrayList historyListView( String apply_code) throws DAOException{
		...
		ArrayList<ProjectDTO> arrlist = new ArrayList<ProjectDTO>();
		try{			
			sbSql.append(" SELECT APPLY_CODE,HISTORY_CODE,PROJECT_NM,START_YMD,END_YMD,ORDER_ORG,PROC_ORG,ROLE,PROJECT_DETAIL ");
			sbSql.append(" FROM T_PROJECT ");
			...
			log.debug("historyListView["+sbSql.toString()+"]");	
			 ...
			 try{
				 ...
				 while(ds.next()){ 
					 ...
					 projectDto.setApply_code(ds.getString("APPLY_CODE"));
					 projectDto.setHistory_code(ds.getString("HISTORY_CODE"));
					 projectDto.setProject_nm(ds.getString("PROJECT_NM"));
					 projectDto.setStart_ymd(ds.getString("START_YMD"));
					 projectDto.setEnd_ymd(ds.getString("END_YMD"));
					 projectDto.setOrder_org(ds.getString("ORDER_ORG"));
					 projectDto.setProc_org(ds.getString("PROC_ORG"));
					 projectDto.setRole(ds.getString("ROLE"));
					 projectDto.setProject_detail(ds.getString("PROJECT_DETAIL"));

					 arrlist.add(projectDto);
				 }
			 }catch(Exception e){
			      e.printStackTrace();
			      log.error(e.getMessage());
