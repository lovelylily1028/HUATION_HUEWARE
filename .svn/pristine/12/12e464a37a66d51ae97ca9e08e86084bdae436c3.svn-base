...
<script type="text/javascript">
	...
	//초기세팅
	function init_newsMagazine() {
		
		formObj=document.newsPageListForm; //form객체세팅
	...
	function goRegist(){
		...
		formObj.action = "<%= request.getContextPath()%>/B_NewsMagazine.do?cmd=newsMagazineRegistForm";
		formObj.submit();
	}
...
<!-- //header -->
	...
	<div id="container">
		<div id="content">
			...
			<h3><span>N</span>ews &amp; Magazine</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
			<div class="con">
				<!-- 컨텐츠 상단 영역 -->
				...
				%>
			  <%=ld.getPageScript("newsPageListForm", "curPage", "goPage")%>
			  <form method="post" name="newsPageListForm" action="<%=request.getContextPath()%>/B_NewsMagazine.do?cmd=newsMagazinePageList" class="search">
				...
				<!-- //컨텐츠 상단 영역 -->
 				...
 				<!-- 리스트 -->
				<table class="tbl_type tbl_type_list" summary="News &amp; Magazine리스트(첨부파일, 게시자, 등록일, 등록시간, 제목, 조회수)">
      ...
      <tbody>
        <tr>
          <td><%
                   String NewsFile=ds.getString("NewsFile");
          		   String NewsFileNm=ds.getString("NewsFileNm");
                   ...
                   //String serverUrl = "http://" + request.getServerName() + ":8088" +request.getContextPath();
                    ...
                    int c_index=NewsFile.lastIndexOf("/");
                  	
                    String newsfilename=NewsFile.substring(c_index+1);
              
                    String newspath=""; //파일경로 읽어오기
                   
                    	
          
                    if(!NewsFile.equals("")){
                    	newspath=NewsFile.substring(0,c_index); //파일경로 읽어오기
                    	...
                    	String spStrReplace = "";
                    	if(NewsFileNm.indexOf("&") != -1){
                    		spStrReplace=  NewsFileNm.replace("&", "[replaceStr]");
                    	System.out.println("spStrReplace:"+spStrReplace);
                    	}else{
                    		spStrReplace =  NewsFileNm;	
                    	System.out.println("spStrReplace:"+spStrReplace) ;
                    	}
                 %>
           
            <a href="<%=serverUrl %>/fileDownServlet?rFileName=<%=spStrReplace%>&sFileName=<%=newsfilename%>&filePath=<%=newspath%>" ><img src="<%= request.getContextPath()%>/images/ico_down.gif" width="16" height="16" align="absmiddle" title="첨부파일"></img></a>
            <%
                     }
