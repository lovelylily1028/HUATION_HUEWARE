...
<script type="text/javascript">
	...
	function goRegist(){
		...
		formObj.action = "<%= request.getContextPath()%>/B_Project_H.do?cmd=projectRegistForm_H";
		formObj.submit();
	}
	...
	function goDetail(Seq){
		 ...
		 formObj.target ='_self';
		 formObj.action = "<%= request.getContextPath()%>/B_Project_H.do?cmd=projectView_H";
	...
	//Excel Export
	function goExcel() {
		 formObj.action = "<%=request.getContextPath()%>/B_Project_H.do?cmd=projectListExcel_H";	
		 formObj.submit();
		 formObj.action = "<%=request.getContextPath()%>/B_Project_H.do?cmd=projectPageList_H";	
	}
...
%>
			...
			<h3><span>정</span>기점검(한솔팩스)</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
  ...
  <form method="post" name="projectPageListForm" action="<%=request.getContextPath()%>/B_Project_H.do?cmd=projectPageList_H" class="search">
    ...
    <!-- search -->
      ...
      <fieldset>
        ...
        <ul>
        	<li>
        		<!-- 달력 시작 -->
                 <span class="ico_calendar"><input name="FrDate"  type="text" value="<%=FrDate %>" class="text textdate" id="calendarData1" readonly="readonly" dispName="날짜" maxlength="8" onKeyUp="if(this.value.length==8) addDateFormat(this);"/><img src="<%=request.getContextPath()%>/images/common/ico_calendar.gif" onclick="showCalendar('1')" alt="달력"/></span>
                    ~ 
                 <span class="ico_calendar"><input name="ToDate"  type="text" value="<%=ToDate %>" class="text textdate" id="calendarData2" readonly="readonly" dispName="날짜" maxlength="8"  onKeyUp="if(this.value.length==8) addDateFormat(this);" /><img src="<%=request.getContextPath()%>/images/common/ico_calendar.gif" onclick="showCalendar('2')" alt="달력"></span>
                <!-- 달력 종료 -->
       		 </li>
     ...
     <div class="Tbtn_areaR"><a href="javascript:goExcel();" class="btn_type01 btn_type01_gray"><span>내려받기</span></a></div><a class="btn_type01 md0" value="등록하기" onclick="javascript:goRegist();"><span>등록하기</span></a></div>
    <!--//search -->

      <table class="tbl_type tbl_type_list" summary="정기점검리스트(한솔팩스)(고객사, 시작일자, 시작시간, 종료일자, 종료시간, 소요시간(Hour), 대상월, 담당자, 고객사담당자, 수행장소, 수행내용, 특이사항, 작성일시, 정기정검보고서)">
        <caption>정기점검리스트</caption>
...
//                    String serverUrl = "https://" + request.getServerName() + ":" + request.getServerPort()+request.getContextPath();
    ...
    <!-- //paginate -->
      ...
      <div class="Bbtn_areaR">
        <a class="btn_type01 md0" value="등록하기" onclick="javascript:goRegist();"><span>등록하기</span></a>
      </div>
    </div>
    <!-- //con -->
