...
function fileCheckNm(szValue){
	...
	//날짜 입력 벨류데이션 체크 시작.
	  var frm = document.ProjectCodeManageEditForm;   //폼셋팅
	...
	//프로젝트 코드 값 조합구분 변경 시 (조합구분 변경 값 + 프로젝트코드 시퀀스 PK) = ProjectCode Value 셋팅.
	...
	function ProjectDivision_Selected(){
		var frm = document.ProjectCodeManageEditForm;
		var ori_PjDivisionChk = $('[name=Ori_ProjectDivision]').val(); 		//기존에 가지고 있던 프로젝트 구분 값 가져오기.
		var ori_MoreCode = $('[name=Ori_MoreCode]').val();					//기존에 가지고 있던 증설코드 가져오기.
		var ori_ProjectCode = $('[name=Ori_ProjectCode]').val();			//기존에 가지고 있던 모 프로젝트 코드 가져오기.
		var ori_ProjectName = $('[name=Ori_ProjectName]').val();			//기존에 가지고 있던 모 프로젝트 명 가져오기.
		var selected_val;
	...
	/* 		$('#PjDivision_td2').attr('colspan','1');	//프로젝트 구분 증설 선택 시  colspan = '2' */
			...
			//프로젝트 구분 증설에서 다른 건으로 변경 후 다시 증설 건으로 돌아 올시 기존 가지고 있던 증설코드 값으로 셋팅해줌.	
			//(선택 값) == (기존 셋팅 값) 비교 후 해당 데이터 셋팅.
			if(selected_val == ori_PjDivisionChk){
			$('#MoreCode').val(ori_MoreCode);						//프로젝트 구분 증설 선택 시 증설코드 name value = 기존 값 셋팅
			$('[name=P_ProjectCode]').val(ori_ProjectCode);			//프로젝트 구분 증설 선택 시 모프로젝트 코드 name value = 기존 값 셋팅
			$('#P_ProjectName').val(ori_ProjectName);				//프로젝트 구분 증설 선택 시 모프로젝트 명 name  value = 기존 값 셋팅
			}else{	
	...
	/* 		$('#PjDivision_td').attr('colspan','1');	//프로젝트 구분 증설 선택(x)  colspan = '4' */
			...
			$('#P_ProjectCode').val('');				//프로젝트 구분 증설 선택(x) 모프로젝트 코드 name  value = '' 초기화
	/* 		$('#PjDivision_td2').attr('colspan','1');	//프로젝트 구분 증설 선택(x)  colspan = '4' */
			...
			$('#MoreCode').val('');						//프로젝트 구분 증설 선택(x) 증설코드 name value = '' 초기화
			$('[name=P_ProjectCode]').val('');			//프로젝트 구분 증설 선택(x) 모프로젝트 코드 name value = '' 초기화
			$('#P_ProjectName').val('');				//프로젝트 구분 증설 선택(x) 모프로젝트 명 name  value = '' 초기화
	...
	/* 		$('#PjDivision_td2').attr('colspan','1');	//프로젝트 구분 증설 선택 시  colspan = '2' */
		...
		/* 	$('#PjDivision_td').attr('colspan','1');	//프로젝트 구분 초기 colspan = '4' */
			$('#MoreCode').hide();						//프로젝트 구분 증설 선택 시 증설코드 name hide
			$('#P_ProjectCode').hide();					//프로젝트 구분 증설 선택 시 모프로젝트 코드 name hide
		/* 	$('#PjDivision_td2').attr('colspan','1');	//프로젝트 구분 초기 colspan = '4' */
			$('#PjDivision_NM_th').hide();				//프로젝트 구분 증설 선택 시 모 프로젝트 명 th hide
	...
	function chkCustomerNameAdd(){
	  ...
	  function datepickerInputText1(){
		  var frm = document.ProjectCodeManageEditForm;	//폼생성
	...
	*/
	  ...
	  function noneContractColum(count){
		...
		function delete_tr(tr_id,cmct,pjnm,pop_ct_a){
			...
			$('#'+tr_id).remove();								//계약코드 메인 테이블 해당 tr 삭제.
			$('#'+cmct).remove();								//계약코드 input 삭제.
			$('#'+pjnm).remove();								//프로젝트명 input 삭제.
		...
		//행 추가
		var paramCnt = 6;	//파라미터 갯수(tr 칼럼 갯수)
							//계약코드 수정 시 삭제 후 다시 추가하여 입력 시 등록처럼 1씩 증가 시키면 계약코드 시퀀스 중복으로 삭제 오류 발생되므로,
							//계약코드 5개 까지 등록되므로 초기 시퀀스 1이 아닌 6으로 셋팅 해놓는다.
		function add_tr(){
			var frm = document.ProjectCodeManageEditForm; 
...
<h3><span>프</span>로젝트코드수정</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
  <!-- //title -->
  <!-- con -->
  <div class="con">
  ...
  <p class="must_txt"><img src="<%=request.getContextPath() %>/images/common/must_icon_01.gif" alt="필수입력별표" />표시는 필수 입력 항목입니다.</p>
	...
	<form name="ProjectCodeManageEditForm" method="post" action="" enctype="multipart/form-data">
		...
		<fieldset>
			<legend>프로젝트코드수정</legend>
		<table class="tbl_type" summary="프로젝트코드수정(프로젝트코드, 프로젝트구분, 발주사구분, 증설코드, 발주사구분, 선택된모프로젝트명, 견적번호, 선택된견적서프로젝트명, 계약코드, 프로젝트명, 고객사명(End User), 발주사명, 프로젝트시작예정일, 프로젝트종료예정일, 프로젝트진행기간, 프로젝트진행요청서, 담당영업, 담당PM, 영업Comments)">
			<caption>프로젝트 코드 등록화면</caption>
				<colgroup>
					<col width="20%" />
					<col width="*" />
				</colgroup>
			...
			<tr>
				<%
					//프로젝트 코드 셋팅.
					String ProjectCode = pjMgDto.getProjectCode();	//객체 셋팅.
					String ProjectCodeSt ="";						//프로젝트 코드 시퀀스PK를 제외한 조합구분 셋팅 변수. 
					String ProjectCodeStNum = "";					//프로젝트 코드 조합구분을 제외한 시퀀스 PK 셋팅 변수.
					ProjectCodeSt= ProjectCode.substring(0,4);		//프로젝트 코드 시퀀스PK를 제외한 조합구분 자르고 변수에 셋팅.
					ProjectCodeStNum = ProjectCode.substring(4,10); //프로젝트 코드 조합구분을 제외한 시퀀스 PK 자르고 변수에 셋팅.
				%>
				...
				<td><input type="text" name="ProjectCode"  class="text dis" title="프로젝트코드" style="width:30px;" readonly="readonly" value="<%=ProjectCodeSt %>"/><br /><span class="guide_txt br">* <strong>프로젝트 코드 조합</strong> : (ex:PJA1) <strong>프로젝트 구분</strong> 및 <strong>발주사 구분</strong>을 반드시 선택하여 조합을 하셔야 합니다.<br />* 조합 후 등록 시 <strong>뒷6자리 코드번호는 자동증가</strong> 생성됩니다. (현재 프로젝트 코드 :<strong><%=ProjectCode %></strong>)</span>
				<!-- 프로젝트 코드 기존에 있던 정보. -->
				<input type="hidden" name="Ori_Pk_ProjectCode" value="<%=pjMgDto.getProjectCode() %>"></input>
				<!-- 프로젝트 코드 기존에 있던 조합구분 정보.-->
				<input type="hidden" name="ProjectCodeSt" value="<%=ProjectCodeSt %>"></input>
				<!-- 프로젝트 코드 기존에 있던 시퀀스 PK 정보. -->
			...
			<tr>
				...
				<td id="PjDivision_td">
					<select name="ProjectDivision_Select" id="SelectBox1" onchange="ProjectDivision_Selected();">
						...
						<option value="A" <%=Division_Pj_Selected_A %>>A-신규</option>
						<option value="B" <%=Division_Pj_Selected_B %>>B-증설</option>
						<option value="C" <%=Division_Pj_Selected_C %>>C-유지보수</option>
					</select>
					<input type="hidden" name="ProjectDivision" value="<%=pjMgDto.getProjectDivision()%>"></input>
					<!-- 기존게 가지고 있던 프로젝트 구분 정보 -->
			...
			<tr>
				...
				<td id="PjDivision_td2">
					<select name="PurchaseDivision_Select" id="SelectBox2" onchange="ProjectDivision_Selected();" title="발주사구분선택">
						<option value="-">선택하세요</option>
						<option value="1" <%=Division_Pc_Selected_1 %>>1. (End User)</option>
						<option value="2" <%=Division_Pc_Selected_2 %>>2. (하도급)</option>
					</select>
			...
			<tr>
				...
				<td><input type="text" name="Public_No" value="<%=pjMgDto.getPublic_No() %>" class="text dis" title="견적번호" style="width:200px;" readonly="readonly"></input><a href="javascript:popPublicNo();" class="btn_type03"><span>견적서조회</span></a></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td id="CtcYN" class="listT01">
					...
					%>
					<dl>
						<dt><input class="radio md0" type="radio" id="CtcdYN" name="CtcdYN" value="Y" <%=Checked_Y %> onclick="javascript:noneContractColum('1');"><label for="">사용</label><input class="radio" type="radio" id="CtcdYN" name="CtcdYN" value="N" <%=Checked_N %> onclick="javascript:noneContractColum('1');"><label for="">미사용</label></dt>
						...
						<dd id="CtcMainTable">
							...
							<table class="tbl_type" summary="계약코드(계약코드, 선택된견적서프로젝트명, 삭제)">
								...
								<tr id="param_tr_<%=pjMgDto_Cm.getSortID() %>">
									<td><input class="text dis" type="text" id="paramCtCd_<%=pjMgDto_Cm.getSortID() %>" name="ContractCode" value="<%=pjMgDto_Cm.getContractCode() %>" style="width:122px;" readonly="readonly"/><a href="javascript:popContractNo('paramCtCd_<%=pjMgDto_Cm.getSortID() %>','paramPjNm_<%=pjMgDto_Cm.getSortID() %>');" class="btn_type03"><span>계약서조회</span></a></td>
									...
									<td class="last text_c">
										<!-- 첫번째(Arr==0)는 디폴트 삭제못함. -->
										<%if(x==0){ %>
										<!-- 두번째(Arr==1)부터 삭제 가능. -->
										<%}else{ %>
										<a href="javascript:delete_tr('param_tr_<%=pjMgDto_Cm.getSortID()%>','paramCtCd_<%=pjMgDto_Cm.getSortID()%>','paramPjNm_<%=pjMgDto_Cm.getSortID()%>','pop_ct_a_<%=pjMgDto_Cm.getSortID()%>');" class="btn_type03"><span>삭제</span></a>
										<%} %> 
										</td>
									<!-- 넘겨줄 정렬순서 -->
			...
			<tr>
				...
				<td>
					<input class="text" type="text" name="PurchaseName" maxlength="50" value="<%=pjMgDto.getPurchaseName()%>" style="width:300px;"></input><!-- 2013_12_27 고객사 동일선택 버튼추가.--><input type="checkbox" name="chkYN" value="N" class="check" onclick="javascript:chkCustomerNameAdd();"></input><label for="">고객사와 동일선택</label></td>
			</tr>
			<tr>
				...
				<td >Date&nbsp;&nbsp;<span class="ico_calendar"><input type="text" size="10" name="ProjectStartDate" id="calendarData1" maxlength="10" value="<%=ProjectStartDate%>" onkeyup="javascript:datepickerInputText1();  dateProgressStatus();" onchange="javascript:dateProgressStatus();" class="text" style="width:100px;" /></span><span class="guide_txt">&nbsp;&nbsp;* 텍스트 상자에 직접 일자 입력 할 경우 (예 : 2013-01-01)</span></td>
			</tr>
			<tr>
				...
				<td id="CmDate_td">Date&nbsp;&nbsp;<span class="ico_calendar"><input type="text" size="10" name="ProjectEndDate" id="calendarData2" class="text" maxlength="10" value="<%=ProjectEndDate%>" onkeyup="javascript:datepickerInputText2(); dateProgressStatus();" onchange="javascript:dateProgressStatus();" class="in_txt" style="width:100px;" /></span><span class="guide_txt">&nbsp;&nbsp;* 텍스트 상자에 직접 일자 입력 할 경우 (예 : 2013-01-01)</span>
			</tr>
			<tr>
				...
				<td><input class="text dis" type="text" name="ProjectProgressDate" style="width:50px;" readonly="readonly" value="<%=pjMgDto.getProjectProgressDate()%>"></input> 일</td>
			...
			<tr>
				...
				<td><div class="fileUp"><input type="text" class="text" id="file1" title="프로젝트진행요청서" style="width:561px;" value="<%=pjMgDto.getProjectProgressFile() %>" /><a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span></a><input type="file" name="ProjectProgressFile" id="upload1" value="<%=pjMgDto.getProjectProgressFile() %>" /></div><span class="guide_txt">&nbsp;&nbsp;* 첨부가능 파일유형 : 모든 파일 / 첨부가능 용량 : 최대 500M</span><input type="hidden" name="p_ProjectProgressFile" value="<%=pjMgDto.getProjectProgressFile() %>"></input><input type="hidden" name="ProjectProgressFileNm" value="<%=pjMgDto.getProjectProgressFileNm() %>"></input></td>
			</tr>
			<tr>
				...
				<td><input type="text" name="user_nm" class="text"style="width:200px;" readOnly value="<%=pjMgDto.getChargeNm()%>" onClick="javascript:popSales();"></input><a href="javascript:popSales();" class="btn_type03"><span>사원조회</span></a></td>
			</tr>
			<tr>
				...
				<td><input type="text" name="user_nm2" class="text"style="width:200px;" readOnly value="<%=pjMgDto.getChargePmNm()%>" onClick="javascript:popSales_Second();"></input><a href="javascript:popSales_Second();" class="btn_type03"><span>사원조회</span></a></td>
			</tr>
			<tr>
				<th><label for="">영업Comments</label></th>
				<td><textarea name="ChargeComent" title="영업Comments" style="width:917px;height:96px;" dispName="영업 Comments " onKeyUp="js_Str_ChkSub('5000', this)"><%=pjMgDto.getChargeComent() %></textarea></td>
			</tr>
			</tbody>
		</table>
		</fieldset>
