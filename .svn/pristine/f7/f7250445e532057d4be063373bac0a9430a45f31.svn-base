...
//조회옵션 초기값	
 ...
 function goSearch() {
	...
	function goExcel() { 
		 formObj.action = "<%=request.getContextPath()%>/B_Invoice.do?cmd=invoiceUnissuedExcelList";	
		 formObj.submit();
		 formObj.action = "<%=request.getContextPath()%>/B_Invoice.do?cmd=invoiceUnissuedList";	
	}
...
<!-- //header -->
	...
	<!-- contents -->
		<div id="content">
			<div class="content_navi">영업지원 &gt; 계산서미발행현황</div>
			<h3><span>계</span>산서미발행현황</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
			<div class="con">
				<!-- 컨텐츠 상단 영역 -->
				<div class="conTop_area">
				  <%=ld.getPageScript("UnissuedListForm", "curPage", "goPage")%>
				<form method ="post" name="UnissuedListForm" action="<%=request.getContextPath()%>/B_Invoice.do?cmd=invoiceUnissuedList"  class="search">
