...
function bluring(){ 
	if(event.srcElement.tagName=="A"||event.srcElement.tagName=="IMG" ||event.srcElement.tagType=="checkbox" ||event.srcElement.tagType=="radio" ||event.srcElement.tagType=="font") document.body.focus(); 
	} 
...
function dateCheck(sObj,eObj,due){
	...
	if(due!= '' && 365<rdue){
    	...
    	alert('검색일수는 '+due+'일로 제한합니다.');
    	return false;
	}*/	
	return true;
}

function dateCheck_5Year(sObj,eObj,due){
	...
	/*var rdue=daysBetween(sdate,edate);
	//var 5Year = 365 * 5 = 1825 + 2; //4년마다 하루(24시간)+5시간이 생기는 윤달이 껴있으므로 365에 +2 더해줌.
	
	if(due!= '' && 1827<rdue){
		alert('검색일수는 5년으로 제한합니다.');
		return false;
	} else if(due!='' && rdue>due){
    	alert('검색일수는 '+due+'년으로 제한합니다.');
    	return false;
	}	*/
	return true;
}
...
var openWin=0;//팝업객체
function imageView(pre_url,UFID,FtpUrl,FtpID,FtpPW,imageFolderName,imageFileName,imageTotalPagecount,Auth,FaxNo,ScreenID){

	var url=pre_url+'&UFID='+UFID+'&FtpUrl='+FtpUrl+'&FtpID='+FtpID+'&FtpPW='+FtpPW+'&imageFolderName='+imageFolderName+'&imageFileName='+imageFileName+'&imageTotalPagecount='+imageTotalPagecount+'&Auth='+Auth+'&FaxNo='+FaxNo+'&ScreenID='+ScreenID;
    var top, left,scroll;
    var width='900';
    var height='600';
    var loc='center';
