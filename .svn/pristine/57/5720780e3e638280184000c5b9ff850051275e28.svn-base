...
function goSave(){
	var frm = document.freeBoardRegist; 
...
/* 	if(fsize > 524288000 ){   //파일용량 체크
	...
	*/     
	...
	function cancle(){
		...
		var frm = document.freeBoardRegist;
		frm.action='<%= request.getContextPath()%>/B_FreeBoard.do?cmd=freeBoardPageList';
		frm.submit();
...
<!-- //header -->
	...
	<div id="container">
		<div id="content">
			...
			<h3><span>자</span>유게시판등록</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
			<div class="con">
				<!-- 컨텐츠 상단 영역 -->
			    ...
			    <form name="freeBoardRegist" method="post" action="<%= request.getContextPath()%>/B_FreeBoard.do?cmd=freeBoardRegist" enctype="multipart/form-data">
			      <input type = "hidden" name = "curPage" value="<%=curPage%>"/>
				  <input type = "hidden" name = "user_id" value="<%=dtoUser.getUserId()%>"/>
      
 				<fieldset>
					<legend>자유게시판등록</legend>
					
					<table class="tbl_type" summary="자유게시판등록(게시자, 첨부파일, 제목, 내용)">
						<colgroup>
							<col width="20%" />
							<col width="*" />
						</colgroup>
						...
						<tr>
							...
							<td><div class="fileUp"><input type="text" class="text" id="file1" title="첨부파일" style="width:561px;" /><a href="#link" class="btn_type03" value="찾아보기"><span>찾아보기</span></a><input type="file" name="BoardFile" id="upload1" /></div><span class="guide_txt">&nbsp;&nbsp;* 첨부가능 파일유형 : 모든 파일 / 첨부가능 용량 : 최대 500M</span></td>
						</tr>
						...
						<tr>
							...
							<td><textarea id="" name="Contents" title="내용" style="width:916px;height:248px;" onKeyUp="js_Str_ChkSub('10000', this)"></textarea></td>
						</tr>
						</tbody>
					</table>
				</fieldset>
