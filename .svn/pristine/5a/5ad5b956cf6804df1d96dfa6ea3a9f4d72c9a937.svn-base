...
<script type="text/javascript">
	...
	//초기세팅
	function init_familyEvent() {
		
		formObj=document.eventPageListForm; //form객체세팅
	...
	function goRegist(){
		...
		formObj.action = "<%= request.getContextPath()%>/B_FamilyEvent.do?cmd=familyEventRegistForm";
		formObj.submit();
	}
...
<!-- //header -->
	...
	<div id="container">
		<div id="content">
			...
			<h3><span>경</span>조사</h3><!-- 타이틀 앞글자는 <span></span>으로 감싸기 -->
			<div class="con">
				<!-- 컨텐츠 상단 영역 -->
				...
				//	UserDTO userDto = (UserDTO) model.get("totalInfo");
					...
					%>
				  <%=ld.getPageScript("eventPageListForm", "curPage", "goPage")%>
				  <form method="post" name=eventPageListForm action="<%=request.getContextPath()%>/B_FamilyEvent.do?cmd=familyEventPageList" class="search">
				...
				<!-- //컨텐츠 상단 영역 -->
				<!-- 리스트 -->
				<table class="tbl_type tbl_type_list" summary="경조사리스트(첨부파일, 게시자, 등록일, 등록시간, 제목, 조회수)">
      ...
      <tbody>  
        <tr>
          <td><%
                   String EventFile=ds.getString("EventFile");
          		   String EventFileNm=ds.getString("EventFileNm");
                   ...
                   //String serverUrl = "http://" + request.getServerName() + ":8088" +request.getContextPath();
                    ...
                    int c_index=EventFile.lastIndexOf("/");
                  	
                    String eventfilename=EventFile.substring(c_index+1);
              
                    String eventpath=""; //파일경로 읽어오기
                   
                    	
          
                    if(!EventFile.equals("")){
                    	eventpath=EventFile.substring(0,c_index); //파일경로 읽어오기
                    	...
                    	String spStrReplace = "";
                    	if(EventFileNm.indexOf("&") != -1){
                    		spStrReplace=  EventFileNm.replace("&", "[replaceStr]");
                    	System.out.println("spStrReplace:"+spStrReplace);
                    	}else{
                    		spStrReplace =  EventFileNm;	
                    	System.out.println("spStrReplace:"+spStrReplace) ;
                    	}
                 %>
           
            <a href="<%=serverUrl %>/fileDownServlet?rFileName=<%=spStrReplace%>&sFileName=<%=eventfilename%>&filePath=<%=eventpath%>" ><img src="<%= request.getContextPath()%>/images/ico_down.gif" width="16" height="16" align="absmiddle" title="첨부파일"></img></a>
            <%
                     }
